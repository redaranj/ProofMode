...
header = ['File Hash SHA256', 'Locale', 'SafetyCheckCtsMatch',
       ...
       'Proof Generated', 'File Path', 'Location.Speed', '']
...
for path, subdir, files in os.walk(PATH):
    for proof_file in glob.glob(path + "\\*" + EXT):
        ...
        current_proof_dic = csv.DictReader(current_proof)
        row2 = next(current_proof_dic)

        with open(newfilename, 'a+', newline='') as csvfile:
            header2 = current_proof_dic.fieldnames
            csvdictwriter = csv.DictWriter(csvfile, fieldnames=header)
            csvdictwriter.writerow(row2)